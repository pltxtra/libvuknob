...
/***************************
 *
 *  Class GnuVGConnector::ConnectionGraphic
 *
 ***************************/

std::map<GnuVGConnector::ConnectionGraphic::cg_machine_cpl,
	 std::shared_ptr<GnuVGConnector::ConnectionGraphic>,
	...
	, destination(_destination)
{
	...
	selectButton.set_event_handler(
		[this, _context](KammoGUI::GnuVGCanvas::SVGDocument *,
...
void GnuVGConnector::ConnectionGraphic::recalculate_position() {
	double dx = x2 - x1;
	double dy = y2 - y1;
...
void GnuVGConnector::ConnectionGraphic::recalculate_end_point(std::shared_ptr<MachineGraphic> end_point) {
...
auto GnuVGConnector::ConnectionGraphic::attach(GnuVGConnector *context,
	...
	SATAN_DEBUG("Create ConnectionGraphic - GnuVGConnector context: %p\n", context);

	KammoGUI::GnuVGCanvas::ElementReference connection_template = KammoGUI::GnuVGCanvas::ElementReference(context, "connectionTemplate");
	KammoGUI::GnuVGCanvas::ElementReference connection_layer = KammoGUI::GnuVGCanvas::ElementReference(context, "connectionContainer");
...
void GnuVGConnector::ConnectionGraphic::debug_print() {

}

void GnuVGConnector::ConnectionGraphic::detach(std::shared_ptr<MachineGraphic> source_machine,
					  std::shared_ptr<MachineGraphic> destination_machine,
					  const std::string &output,
					  const std::string &input) {
...
void GnuVGConnector::ConnectionGraphic::hide_all() {
...
void GnuVGConnector::ConnectionGraphic::show_all() {
	for(auto g : connection_graphics) {
...
void GnuVGConnector::ConnectionGraphic::machine_unregistered(std::shared_ptr<MachineGraphic> machine) {
...
/***************************
 *
 *  Class GnuVGConnector::MachineGraphic::Transition
 *
 ***************************/

GnuVGConnector::MachineGraphic::Transition::Transition(
	GnuVGConnector::MachineGraphic *_context,
	std::function<void(GnuVGConnector::MachineGraphic *context, float progress)> _callback) :
	KammoGUI::Animation(TRANSITION_TIME), ctx(_context), callback(_callback) {
}

void GnuVGConnector::MachineGraphic::Transition::new_frame(float progress) {
	callback(ctx, progress);
}

void GnuVGConnector::MachineGraphic::Transition::on_touch_event() { /* ignored */ }

/***************************
 *
 *  Class GnuVGConnector::MachineGraphic
 *
 ***************************/
...
GnuVGConnector::MachineGraphic::IOSocket::IOSocket(KammoGUI::GnuVGCanvas::ElementReference original,
					      KammoGUI::GnuVGCanvas::ElementReference _socket_gfx,
					      MachineGraphic *_owner, const std::string &_name) :
	KammoGUI::GnuVGCanvas::ElementReference(&original), socket_gfx(_socket_gfx), owner(_owner), name(_name)
{}

void GnuVGConnector::MachineGraphic::debug_print() {
...
void GnuVGConnector::MachineGraphic::hide_sockets(std::vector<IOSocket *> &vctr) {
	for(auto socket : vctr) {
		socket->socket_gfx.set_display("none");
	}
}

void GnuVGConnector::MachineGraphic::show_sockets(std::vector<IOSocket *> &vctr) {
	for(auto socket : vctr) {
		socket->socket_gfx.set_display("inline");
	}
}

void GnuVGConnector::MachineGraphic::create_sockets(std::vector<IOSocket *> &vctr, std::vector<std::string> names,
					       KammoGUI::GnuVGCanvas::ElementReference &template_socket,
					       void (*this_on_event)(KammoGUI::GnuVGCanvas::SVGDocument *source,
	...
	for(auto name : names) {
		...
		try {
			KammoGUI::GnuVGCanvas::ElementReference socket =
			...
			KammoGUI::GnuVGCanvas::SVGMatrix rotate_t;
			...
			angle += angle_step;
		} catch(KammoGUI::GnuVGCanvas::OperationFailedException &e) {
...
void GnuVGConnector::MachineGraphic::select(const std::string &output_socket_name) {
...
void GnuVGConnector::MachineGraphic::deselect() {
	if(current_output) {
...
void GnuVGConnector::MachineGraphic::refresh_appearance(double progress) {
	if(!detailed_mode) progress = 1.0 - progress; // inverted value

	{
		double scale = 0.5 + progress * 0.5;

		KammoGUI::GnuVGCanvas::SVGMatrix zoom_t;
...
void GnuVGConnector::MachineGraphic::refresh_position(double temp_x, double temp_y) {
...
void GnuVGConnector::MachineGraphic::transition_progressed(MachineGraphic *ctx, float progress) {
	ctx->refresh_appearance((double)progress);
}

void GnuVGConnector::MachineGraphic::on_event(KammoGUI::GnuVGCanvas::SVGDocument *source,
					 KammoGUI::GnuVGCanvas::ElementReference *e_ref,
					 const KammoGUI::MotionEvent &event) {
...
void GnuVGConnector::MachineGraphic::on_output_socket_event(KammoGUI::GnuVGCanvas::SVGDocument *source,
						       KammoGUI::GnuVGCanvas::ElementReference *e_ref,
						       const KammoGUI::MotionEvent &event) {
	if(event.get_action() == KammoGUI::MotionEvent::ACTION_UP) {
...
void GnuVGConnector::MachineGraphic::on_input_socket_event(KammoGUI::GnuVGCanvas::SVGDocument *source,
						      KammoGUI::GnuVGCanvas::ElementReference *e_ref,
						      const KammoGUI::MotionEvent &event) {
	if(event.get_action() == KammoGUI::MotionEvent::ACTION_UP) {
		IOSocket *io_sock = (IOSocket *)e_ref;
...
std::shared_ptr<GnuVGConnector::MachineGraphic> GnuVGConnector::MachineGraphic::create(GnuVGConnector *context,
	...
	KammoGUI::GnuVGCanvas::ElementReference graphic_template = KammoGUI::GnuVGCanvas::ElementReference(context, "machineGraphicTemplate");
	KammoGUI::GnuVGCanvas::ElementReference connector_layer = KammoGUI::GnuVGCanvas::ElementReference(context, "connectorContainer");
...
void GnuVGConnector::MachineGraphic::hide() {
	set_display("none");
}

void GnuVGConnector::MachineGraphic::show() {
	set_display("inline");
}

void GnuVGConnector::MachineGraphic::leave_detailed_mode() {
	detailed_mode = false;
...
void GnuVGConnector::MachineGraphic::get_position(double &_x, double &_y) {
	_x = appear_at_x;
	_y = appear_at_y;
}

void GnuVGConnector::MachineGraphic::animate_selection() {
	if(current_output) {
		if(auto mgfx = current_output->first.lock()) {
			if(mgfx->selected_animation) {
				KammoGUI::GnuVGCanvas::SVGMatrix transform_m = mgfx->selection_indicator_base_t;
...
/***************************
 *
 *  Class GnuVGConnector::ZoomTransition
 *
 ***************************/

GnuVGConnector::ZoomTransition::ZoomTransition(
	GnuVGConnector *_context,
	std::function<void(GnuVGConnector *context, double zoom, double x, double y)> _callback,
...
void GnuVGConnector::ZoomTransition::new_frame(float progress) {
...
void GnuVGConnector::ZoomTransition::on_touch_event() { /* ignored */ }
...
void GnuVGConnector::on_render() {
	...
	KammoGUI::GnuVGCanvas::ElementReference(this, "connectionContainer").set_transform(transform_m);
	KammoGUI::GnuVGCanvas::ElementReference(this, "connectorContainer").set_transform(transform_m);

	MachineGraphic::animate_selection();
}

void GnuVGConnector::on_resize() {
...
void GnuVGConnector::zoom_callback(GnuVGConnector *ctx, double zoom, double x, double y) {
	ctx->scaling = zoom;
	ctx->position_x = x;
	ctx->position_y = y;
}

bool GnuVGConnector::on_scale(KammoGUI::ScaleGestureDetector *detector) {
	pan_zoom_scale *= (double)detector->get_scale_factor();

	return true;
}

bool GnuVGConnector::on_scale_begin(KammoGUI::ScaleGestureDetector *detector) {
...
void GnuVGConnector::on_scale_end(KammoGUI::ScaleGestureDetector *detector) {
...
void GnuVGConnector::on_pan_and_zoom_event(KammoGUI::GnuVGCanvas::SVGDocument *source,
				      KammoGUI::GnuVGCanvas::ElementReference *e_ref,
				      const KammoGUI::MotionEvent &event) {
	GnuVGConnector *ctx = (GnuVGConnector *)source;
...
GnuVGConnector::GnuVGConnector(KammoGUI::GnuVGCanvas *cnvs)
	: SVGDocument(std::string(SVGLoader::get_svg_directory() + "/connector.svg"), cnvs)
	, plus_button(cnvs, std::string(SVGLoader::get_svg_directory() + "/plusButton.svg"), GnuVGCornerButton::bottom_right)
	, trash_button(cnvs, std::string(SVGLoader::get_svg_directory() + "/trashButton.svg"), GnuVGCornerButton::top_right)
	, settings_button(cnvs, std::string(SVGLoader::get_svg_directory() + "/settingsButton.svg"), GnuVGCornerButton::top_left)
	, list_view(cnvs)
	, pan_and_zoom(this, "panAndZoomSurface")
	, position_x(0.0)
	, position_y(0.0)
	, pan_zoom_offset_x(0.0)
	, pan_zoom_offset_y(0.0)
	, pan_zoom_scale(1.0)
	, sgd(this) {

	connection_list = std::make_shared<GnuVGConnectionList>(cnvs);

	// hide template graphics
	KammoGUI::GnuVGCanvas::ElementReference(this, "machineGraphicTemplateContainer").set_display("none");
...
GnuVGConnector::~GnuVGConnector() {}

double GnuVGConnector::get_scaling() {return scaling;}
...
void GnuVGConnector::zoom_restore() {
