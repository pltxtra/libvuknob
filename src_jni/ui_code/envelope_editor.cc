...
/***************************
 *
 *  Class EnvelopeEditor::ControlPoint
 *
 ***************************/

EnvelopeEditor::ControlPoint::ControlPoint(KammoGUI::SVGCanvas::SVGDocument *context, const std::string &id, int _lineNtick) :
...
std::pair<int, int> EnvelopeEditor::get_envelope_coordinates(float x, float y) {
...
bool EnvelopeEditor::on_scale(KammoGUI::ScaleGestureDetector *detector) {
...
bool EnvelopeEditor::on_scale_begin(KammoGUI::ScaleGestureDetector *detector) { return true; }
void EnvelopeEditor::on_scale_end(KammoGUI::ScaleGestureDetector *detector) { }

void EnvelopeEditor::clear_envelope_svg() {
...
void EnvelopeEditor::generate_envelope_svg() {
	clear_envelope_svg();
...
void EnvelopeEditor::update_envelope() {
...
void EnvelopeEditor::refresh_enable_toggle() {
	KammoGUI::SVGCanvas::SVGMatrix mtrx;
	mtrx.init_identity();
...
bool EnvelopeEditor::refresh_envelope() {
...
void EnvelopeEditor::set_controller_name(const std::string &name) {
	KammoGUI::SVGCanvas::ElementReference controller_ref(this, "controllerId");
	controller_ref.set_text_content(name);
	controller_name = name;
}

void EnvelopeEditor::listview_callback(void *context, bool row_selected, int row_index, const std::string &text) {
	EnvelopeEditor *ctx = (EnvelopeEditor *)context;
...
EnvelopeEditor::EnvelopeEditor(KammoGUI::SVGCanvas *cnvs, std::string fname) : SVGDocument(fname, cnvs), actual_envelope(NULL), current_mode(scroll_and_zoom), zoom_factor(1.0), offset(0.0) {
...
void EnvelopeEditor::set_machine_sequencer(MachineSequencer *_m_seq) {
	m_seq = _m_seq;
...
static EnvelopeEditor *c_env = NULL;

KammoEventHandler_Declare(EnvelopeEditorHandler,"controllerEnvelope:showControllerEnvelope");
...
virtual void on_init(KammoGUI::Widget *wid) {
	if(wid->get_id() == "controllerEnvelope") {
		...
		c_env = new EnvelopeEditor(cnvs, std::string(SVGLoader::get_svg_directory() + "/controllerEnvelope.svg"));
	}
}
...
KammoEventHandler_Instance(EnvelopeEditorHandler);
