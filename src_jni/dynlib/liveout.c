...
static AndroidInstance *init_android(MachineTable *mt) {
	DYNLIB_INFORM("Initiating standard Android Audio, using AudioTrack.\n");
	...
	if(!valid_instance) {
		...
		RIFF_prepare(inst, period_size * 2, &(inst->riff_file), rate);
	} else {
...
OpenSLInstance *init_OpenSL(MachineTable *mt) {
	...
	case __PLAYBACK_OPENSL_BUFFERED:
	{
		...
		DYNLIB_INFORM("Initiating standard Android OpenSL ES Audio, using buffered mode.\n");
	...
	case __PLAYBACK_OPENSL_DIRECT:
	{
		DYNLIB_INFORM("Initiating standard Android OpenSL ES Audio, using direct mode.\n");
	...
	case __PLAYBACK_SAMSUNG:
	{
		DYNLIB_INFORM("Initiating Android Audio, using Samsung Professional Audio.\n");
	...
	if(inst->playback_mode != __PLAYBACK_SAMSUNG) {
		...
		if((buffers != NULL) && (_empty_buffer != NULL)) {
			...
			if((inst->stream = android_OpenAudioDevice(rate, 2, period_size, number_of_buffers)) != NULL) {
				...
				RIFF_prepare(inst, __opensl_buffer_factor * period_size * 2, &(inst->riff_file), rate);
			} else {
...
void samsung_thread_callback(unsigned int framesize, unsigned int frequency,
	...
	if(inst->period_size != framesize) {
		...
		RIFF_prepare(inst, inst->period_size * 2, &(inst->riff_file), frequency);
	}
