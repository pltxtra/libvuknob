...
/***************************
 *
 *  Class GnuVGListView::Row
 *
 ***************************/

void GnuVGListView::Row::on_event(KammoGUI::GnuVGCanvas::SVGDocument *source,
...
GnuVGListView::Row::Row(GnuVGListView *_parent, const std::string &_text, int _row_index, const std::string &id) :
...
GnuVGListView::Row *GnuVGListView::Row::create(GnuVGListView *parent, int row_index, const std::string &text) {
	// locate needed elements
	KammoGUI::GnuVGCanvas::ElementReference base_element = KammoGUI::GnuVGCanvas::ElementReference(parent, "listViewRow");
	KammoGUI::GnuVGCanvas::ElementReference container = KammoGUI::GnuVGCanvas::ElementReference(parent, "listContainer");

	// create a new id for the row
	std::stringstream new_id;
	new_id << "listViewRow__" << row_index;

	SATAN_DEBUG("GnuVGListView::Row::Create(%d, %s) -> %s\n", row_index, text.c_str(), new_id.str().c_str());
...
void GnuVGListView::hide() {
	KammoGUI::GnuVGCanvas::ElementReference root_element = KammoGUI::GnuVGCanvas::ElementReference(this);
	root_element.set_display("none");
}

void GnuVGListView::on_cancel_event(KammoGUI::GnuVGCanvas::SVGDocument *source,
				    KammoGUI::GnuVGCanvas::ElementReference *e_ref,
				    const KammoGUI::MotionEvent &event) {
	GnuVGListView *ctx = (GnuVGListView *)source;
...
void GnuVGListView::row_selected(int row_index, const std::string &selected_text) {
	hide();
...
GnuVGListView::GnuVGListView(KammoGUI::GnuVGCanvas *cnv) : SVGDocument(std::string(SVGLoader::get_svg_directory() + "/listView.svg"), cnv), offset(0.0) {
	...
	KammoGUI::GnuVGCanvas::ElementReference(this, "listViewRow").set_display("hidden");
}

void GnuVGListView::on_resize() {
...
void GnuVGListView::on_render() {
	...
	{
		// Translate GnuVGListViewBody, and scale it properly to fit the defined viewbox
...
void GnuVGListView::clear() {
	for(auto row : rows) {
		SATAN_DEBUG("GnuVGListView::clear() - Dropping element.\n");
		row->drop_element();
		delete row;
	}
	rows.clear();
	SATAN_DEBUG("GnuVGListView::clear() -> dump id table.");
}

void GnuVGListView::add_row(const std::string &new_row_text) {
	rows.push_back(Row::create(this, rows.size(), new_row_text));
}

void GnuVGListView::select_from_list(const std::string &title,
